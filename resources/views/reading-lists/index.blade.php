        ...
        <section class="mx-auto max-w-4xl space-y-5">
            <div class="rounded-2xl border border-[#eadfce] bg-white p-5 dark:border-[#27211a] dark:bg-[#090909]">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-[0.24em] text-[#9b6b3f]">Biblioteca pessoal</p>
                    ...
                    <form method="POST" action="{{ route('reading-lists.store') }}" class="flex gap-2">
                        ...
                        <button type="submit" class="shrink-0 rounded-lg bg-[#FE6807] px-4 py-2 text-sm font-semibold text-white hover:bg-[#e55c00]">Criar</button>
                    </form>
                </div>
            </div>
                ...
                <div class="rounded-2xl border border-[#eadfce] bg-white p-5 dark:border-[#27211a] dark:bg-[#090909]">
                    <div class="flex items-center justify-between gap-3">
                        <h2 class="text-lg font-semibold text-[#241b14] dark:text-white">{{ $list->name }}
                            <span class="ml-1 text-sm font-normal text-[#66594d] dark:text-[#cfc5ba]">({{ $list->items_count }})</span>
                        </h2>
                        <form method="POST" action="{{ route('reading-lists.destroy', $list->id) }}">
                            @csrf @method('DELETE')
                            <button type="submit" class="text-xs text-red-500 hover:underline" onclick="return confirm('Eliminar lista?')">Eliminar</button>
                        </form>
                    </div>
                    ...
                    @else
                        <ul class="mt-3 space-y-2">
                            @foreach ($list->items as $item)
                                ...
                                <li class="flex items-center justify-between gap-3 rounded-lg border border-[#eadfce] bg-[#fffaf5] px-3 py-2 dark:border-[#332820] dark:bg-[#0f0d0b]">
                                    <div class="min-w-0">
                                        <a href="{{ $resource?->slug ? route('resources.public.show', $resource->slug) : '#' }}" class="truncate font-medium text-[#241b14] hover:underline dark:text-white">
                                            {{ $resource?->title ?? 'Recurso removido' }}
                                        </a>
                                        <p class="text-xs text-[#9b8b7c]">{{ ucfirst($resource?->type ?? '') }}</p>
                                    </div>
                                    <form method="POST" action="{{ route('reading-lists.items.remove', [$list->id, $item->resource_id]) }}">
                                        @csrf @method('DELETE')
            ...
            @empty
                <div class="rounded-2xl border border-[#eadfce] bg-white p-8 text-center dark:border-[#27211a] dark:bg-[#090909]">
                    <p class="text-[#66594d] dark:text-[#cfc5ba]">Ainda não tem listas. Crie uma acima ou adicione recursos directamente.</p>
                </div>
            @endforelse
        </section>
    </main>
