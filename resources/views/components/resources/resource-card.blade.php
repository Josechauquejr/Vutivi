...
<article data-resource-card data-resource-id="{{ $resource['id'] }}"
    ...
    <div
        class="relative h-40 overflow-hidden rounded-lg bg-gradient-to-br {{ $resource['cover_class'] }} p-4 text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.24)] sm:h-44 md:h-auto md:min-h-60 md:w-36 md:flex-none lg:w-40">
    ...
    <div class="relative flex flex-1 flex-col gap-4 px-1 pb-1 pt-4 md:min-w-0 md:pt-1">
        <div class="flex-1">
            <div class="flex flex-wrap items-start justify-between gap-3">
                ...
                <span class="rounded-full border px-3 py-1 text-xs font-semibold {{ $isAvailable ? 'border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-900/50 dark:bg-emerald-950/20 dark:text-emerald-200' : 'border-[#d8c5ad] bg-[#f8f0e8] text-[#6a4d38] dark:border-[#332820] dark:bg-[#15120f] dark:text-[#d8cec3]' }}">
                    {{ $resource['available_count'] }} disponíveis
                </span>
            </div>
        ...
        <div class="flex flex-col gap-2 border-t border-[#f3e4d8] pt-3 dark:border-[#241915] sm:flex-row">
            <a href="{{ $accessUrl }}" {{ $isDigital ? 'data-download-action' : '' }}
                ...
                <div id="resource-actions-{{ $resource['id'] }}" class="z-40 hidden w-56 rounded-2xl border border-[#eadfce] bg-white p-2 shadow-[0_18px_48px_rgba(54,39,25,0.14)] dark:border-[#2b211b] dark:bg-[#0b0908]">
                    @auth
                        <form data-favorite-form method="POST" action="{{ $resource['favorite_route'] }}">
                            @csrf
                            <button data-favorite-button class="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold text-[#5f4632] hover:bg-[#fff7f0] dark:text-[#d8cec3] dark:hover:bg-[#15110e]">
                                <span data-favorite-label>{{ ($resource['is_favorited'] ?? false) ? 'Remover favorito' : 'Favoritar' }}</span>
                            </button>
                        </form>
                    @endauth
