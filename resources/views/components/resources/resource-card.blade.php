...
<article data-resource-card data-resource-id="{{ $resource['id'] }}"
    class="group relative flex flex-col overflow-hidden rounded-xl border border-[#eadfce] bg-white p-3 shadow-[0_16px_34px_rgba(54,39,25,0.07)] transition duration-300 hover:-translate-y-1 hover:border-[#d8c5ad] hover:shadow-[0_24px_52px_rgba(54,39,25,0.12)] dark:border-[#27211a] dark:bg-[#090909] dark:shadow-[0_18px_34px_rgba(0,0,0,0.30)] dark:hover:border-[#534337] md:flex-row md:gap-4">

    <div class="relative h-40 overflow-hidden rounded-lg bg-gradient-to-br {{ $resource['cover_class'] }} p-4 text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.24)] sm:h-44 md:h-auto md:min-h-60 md:w-36 md:flex-none lg:w-40">
        ...
        <div class="relative flex h-full flex-col justify-between">
            <div class="flex items-start justify-between gap-3 md:flex-col md:gap-2 xl:flex-row">
                <span class="rounded-full bg-white/15 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.32em] text-white/90 backdrop-blur">{{ $resource['tag'] }}</span>
                <span class="rounded-full border border-white/20 bg-black/10 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.25em] text-white/75">{{ $resource['category_label'] }}</span>
            </div>

            <div>
                <p class="whitespace-pre-line text-[1.1rem] font-semibold leading-[1.15] tracking-tight sm:text-[1.25rem]">{{ $resource['cover_title'] }}</p>
                <p class="mt-3 text-[10px] uppercase tracking-[0.28em] text-white/75">{{ $resource['cover_author'] }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="relative flex flex-1 flex-col gap-4 px-1 pb-1 pt-4 md:min-w-0 md:pt-1">
        <div class="flex-1">
            ...
            <p class="mt-3 text-sm leading-6 text-[#7a5c4a] dark:text-[#d5c7be] md:line-clamp-3">{{ $resource['description'] }}</p>
        </div>

        <div class="flex flex-col gap-2 border-t border-[#f3e4d8] pt-3 dark:border-[#241915] sm:flex-row">
            <a href="{{ $accessUrl }}" class="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-lg border border-[#ffd2b2] bg-[#fff4ec] px-5 text-sm font-semibold text-[#b85214] shadow-[0_8px_18px_rgba(88,44,14,0.08)] transition hover:border-[#f7b184] hover:bg-[#ffe8d8] hover:text-[#8f3f0f] dark:border-[#4a2a1b] dark:bg-black dark:text-[#ffb07a] dark:hover:border-[#FE6807] dark:hover:bg-[#120d0a]">
            ...
            @auth
                <form data-favorite-form method="POST" action="{{ $resource['favorite_route'] }}">
                    @csrf
                    <button data-favorite-button class="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-lg border border-[#decbb8] bg-white px-4 text-sm font-bold text-[#5f4632] transition hover:border-[#FE6807] hover:text-[#FE6807] dark:border-[#332820] dark:bg-[#050505] dark:text-[#d8cec3]" title="{{ $favoriteLabel }}">
            ...
            <div class="relative">
                <button data-dropdown-toggle="resource-actions-{{ $resource['id'] }}" data-dropdown-placement="bottom-end" class="inline-flex min-h-11 w-full items-center justify-center rounded-lg border border-[#decbb8] bg-white px-4 text-[#5f4632] transition hover:bg-[#fff7f0] dark:border-[#332820] dark:bg-[#050505] dark:text-[#d8cec3] dark:hover:bg-[#15120f]" title="Mais ações">
