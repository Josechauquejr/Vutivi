...
<x-head title="Editar recurso digital" />
...
<body class="home-layout bg-[#fbfaf7] dark:bg-[#050505]">
    ...
    <main class="item px-3 pb-10 pt-4 sm:px-5 md:px-6 lg:px-8">
        <section class="surface-card mx-auto max-w-7xl p-6 dark:border-[#27211a] dark:bg-[#090909] md:p-8">
            ...
            <h1 class="mt-2 text-3xl font-semibold text-[#241b14] dark:text-white">Editar recurso digital</h1>
            ...
            <form method="POST" action="{{ route('digital-resources.update', $resource->id) }}" enctype="multipart/form-data" class="mt-6 grid gap-6">
                @csrf
                @method('PUT')
                @include('digital_resources.form', ['resource' => $resource, 'digital' => $resource->digitalResource, 'buttonLabel' => 'Atualizar recurso digital'])
            </form>
        </section>
    </main>
</body>
