...
<body class="home-layout bg-[#fbfaf7] dark:bg-[#050505]">
    ...
    <main class="item px-3 pb-10 pt-4 sm:px-5 md:px-6 lg:px-8">
        ...
        <section class="mx-auto max-w-5xl space-y-5">
            ...
            @if (session('error'))
                <div
                    class="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700 dark:border-red-900/40 dark:bg-red-950/20 dark:text-red-300">
                    {{ session('error') }}</div>
            @endif

            {{-- Perfil --}}
            <div
                class="relative overflow-hidden rounded-2xl border border-[#eadfce] bg-white p-6 dark:border-[#27211a] dark:bg-[#090909]">
                <div
                    class="pointer-events-none absolute -right-8 -top-8 h-40 w-40 rounded-full bg-[#FE6807]/5 blur-2xl dark:bg-[#FE6807]/10">
                </div>
                <div class="relative flex flex-col gap-5 sm:flex-row sm:items-start">
                    ...
                    <div class="shrink-0">
                        ...
                        @else
                            ...
                            @endphp
                            <div
                                class="flex h-20 w-20 items-center justify-center rounded-full bg-[#FE6807] text-2xl font-black text-white ring-4 ring-[#fff1e6] dark:ring-[#17120f]">
                                {{ $initials }}
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            @if ($user->isAdmin())
                                <span
                                    class="rounded-full bg-red-100 px-2.5 py-0.5 text-xs font-black text-red-700 dark:bg-red-950/40 dark:text-red-300">Admin</span>
                            @elseif ($user->isLibrarian())
                                <span
                                    class="rounded-full bg-[#fff1e6] px-2.5 py-0.5 text-xs font-black text-[#9b6b3f] dark:bg-[#17120f]">Bibliotecário</span>
                            @endif
                        </div>
                ...
                <div class="mt-6 grid grid-cols-3 gap-4">
                    <div
                        class="rounded-xl border border-[#eadfce] bg-[#fffaf5] p-4 text-center dark:border-[#27211a] dark:bg-[#0f0d0b]">
                ...
                <div class="mt-4 grid grid-cols-4 gap-2">
                    @foreach ($quickLinks as $ql)
                        <a href="{{ route($ql['route']) }}"
                            ...
                            <svg class="h-4 w-4 shrink-0 opacity-70" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">{!! $ql['svg'] !!}</svg>
                            {{ $ql['label'] }}
                        </a>
                    @endforeach
                </div>
            ...
            <div class="rounded-2xl border border-red-200 bg-white dark:border-red-900/30 dark:bg-[#090909]">
                <div class="p-5">
                    <div class="flex items-start gap-3">
                        <div
                            class="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-red-50 dark:bg-red-950/30">
                            <svg class="h-4 w-4 text-red-600 dark:text-red-400" viewBox="0 0 24 24" fill="none"
                        ...
                        <div>
                            ...
                            <p class="mt-0.5 text-sm text-[#66594d] dark:text-[#cfc5ba]">
                                ...
                                Todos os seus dados serão apagados: recursos, reservas, listas de leitura, avaliações e
                                histórico.
                            </p>
                        </div>
                    </div>
                ...
                <div class="border-t border-red-100 dark:border-red-900/20">
                    <details class="group">
                        <summary
                            ...
                            <svg class="h-4 w-4 text-red-500 transition duration-200 group-open:rotate-180"
                        ...
                        <div
                            class="border-t border-red-100 bg-red-50/50 px-5 py-5 dark:border-red-900/20 dark:bg-red-950/10">
                            @if ($errors->has('current_password'))
                                <div
                                    class="mb-4 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm font-semibold text-red-700 dark:border-red-900/40 dark:bg-red-950/30 dark:text-red-300">
                            ...
                            <form method="POST" action="{{ route('users.destroy') }}"
                                ...
                                <div class="flex flex-col gap-3 sm:flex-row sm:items-end">
                                    <label class="flex-1">
                                        <span
                                            class="mb-2 block text-sm font-semibold text-[#241b14] dark:text-white">Password
                                            atual</span>
                                        <div
                                            class="flex overflow-hidden rounded-xl border border-[#decbb8] bg-[#fffaf5] transition focus-within:border-[#FE6807] focus-within:ring-2 focus-within:ring-[#FE6807]/15 dark:border-[#332820] dark:bg-[#0d0b09]">
                                            <span
